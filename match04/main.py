...
def get_none_display_key(key, value):
    """调用node.js接口计算j_key"""

    after_encrypt_key = requests.post(url='http://localhost:8888/encrypt',
                                      data={'encrypt_key': key, 'encrypt_value': value}).text

    return after_encrypt_key


def request_page_info(page):
    """从服务器请求数据"""

    url = f'http://match.yuanrenxue.com/api/match/4?page={str(page)}'
...
def get_ans(page_num):
    """主函数，分别请求数据，然后换算成正确数字，求和"""
    ans = 0

    for page in range(1, page_num + 1):
        page_info = request_page_info(page)

        key = page_info['key']
        value = page_info['value']
        none_display_key = get_none_display_key(key, value)
        print(none_display_key)
        tds = re.findall('<td>(.*?)</td>', page_info['info'], re.S)

        for td in tds:
            ans += img_to_num(td, none_display_key)

    return ans


def img_to_num(td, none_display_key):
    """把img标签转换成正确数字"""
    # 去除不显示的图片
    ...
    for img in imgs:
        # 获取图片及偏移量
        img_base64 = re.findall(r'base64,(.*?)" class', img, re.S)[0]
        style = re.findall(r'style="left:(.*?)px"', img, re.S)[0]
        # 除以11是img的宽度是11px
        num_list.append((int(float(style)) // 11, num_base64_dict[img_base64]))

    # print(num_list)
    real_num = num_list_to_num(num_list)

    return real_num


def num_list_to_num(num_list):
    """通过偏移量计算正确的数字"""
    # 在偏移量的基础上加上img占据对应的宽度
    true_list = [(nums[0] + idx, nums[1]) for idx, nums in enumerate(num_list)]
    true_list.sort(key=lambda x: x[0])

    num = 0
    # print(true_list)
    for nums in true_list:
        num = num * 10 + nums[1]
